...
import { getUsuarios } from '@/features/admin/services/usuariosService'
...
const ROLES_ESTRUCTURA = ['ADMIN', 'COORDINADOR', 'PROFESOR', 'NEXO']
const ROLES_USUARIOS = ['ADMIN', 'COORDINADOR', 'NEXO']
const ROLES_TAREAS = ['ADMIN', 'COORDINADOR', 'PROFESOR']

export function DashboardPage() {
  const { user } = useAuth()

  const canVerEstructura = user?.roles?.some((r) => ROLES_ESTRUCTURA.includes(r)) ?? false
  const canVerUsuarios = user?.roles?.some((r) => ROLES_USUARIOS.includes(r)) ?? false
  const canGestionarTareas = user?.roles?.some((r) => ROLES_TAREAS.includes(r)) ?? false

  const { data: usuarios = [] } = useQuery({
    queryKey: ['admin-usuarios'],
    queryFn: getUsuarios,
    enabled: canVerUsuarios,
  })
  const { data: materias = [] } = useQuery({
    queryKey: ['materias'],
    queryFn: getMaterias,
    enabled: canVerEstructura,
  })
  const { data: cohortes = [] } = useQuery({
    queryKey: ['cohortes', undefined],
    queryFn: () => getCohortes(),
    enabled: canVerEstructura,
  })
