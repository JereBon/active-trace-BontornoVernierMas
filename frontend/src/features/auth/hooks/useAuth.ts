...
import { loginApi, refreshApi, verifyTwoFaApi } from '@/features/auth/services/authService'
import {
  isAuthChallenge,
  type AuthChallenge,
  type LoginRequest,
...
export function AuthProvider({ children }: AuthProviderProps) {
  ...
  const verify2fa = useCallback(async (code: string): Promise<void> => {
    ...
    const tokens = await verifyTwoFaApi(challenge.challenge_token, code)
    setAccessToken(tokens.access_token)
    localStorage.setItem('rt', tokens.refresh_token)
    setUser(decodeUserFromToken(tokens.access_token))
    setIsAuthenticated(true)
    setChallenge(null)
  }, [challenge])
