...
// Aligned with backend EvaluacionCreate schema
import { useForm } from 'react-hook-form'
import { zodResolver } from '@hookform/resolvers/zod'
import { z } from 'zod'
import { useQuery } from '@tanstack/react-query'
import { getMaterias, getCohortes } from '@/features/admin/services/estructuraService'
import type { EvaluacionCreate, TipoEvaluacion } from '../../types'
...
const schema = z.object({
  ...
  tipo: z.enum(TIPOS_EVALUACION),
  instancia: z.string().min(1, 'La instancia es requerida'),
  dias_disponibles: z.coerce.number().int().min(1, 'Al menos 1 día'),
  cupos_disponibles: z.coerce.number().int().min(1, 'Al menos 1 cupo'),
})

type FormValues = z.infer<typeof schema>

interface Props {
  onSubmit: (data: EvaluacionCreate) => void
  onCancel: () => void
  isLoading?: boolean
}
...
export function ColoquioForm({ onSubmit, onCancel, isLoading }: Props) {
  ...
  const {
    ...
    defaultValues: { materia_id: '', cohorte_id: '', tipo: 'Coloquio', instancia: '', dias_disponibles: 7, cupos_disponibles: 10 },
  })
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div>
          <label htmlFor="instancia" className="block text-sm font-medium text-gray-700 mb-1">Instancia</label>
          <input id="instancia" {...register('instancia')} className="w-full border border-gray-300 rounded px-3 py-2 text-sm" placeholder="Ej: Coloquio Final" />
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label htmlFor="dias_disponibles" className="block text-sm font-medium text-gray-700 mb-1">Días disponibles</label>
          <input id="dias_disponibles" type="number" min={1} {...register('dias_disponibles')} className="w-full border border-gray-300 rounded px-3 py-2 text-sm" />
          {errors.dias_disponibles && <p className="text-red-600 text-xs mt-1">{errors.dias_disponibles.message}</p>}
        </div>
        <div>
          <label htmlFor="cupos_disponibles" className="block text-sm font-medium text-gray-700 mb-1">Cupos disponibles</label>
          <input id="cupos_disponibles" type="number" min={1} {...register('cupos_disponibles')} className="w-full border border-gray-300 rounded px-3 py-2 text-sm" />
