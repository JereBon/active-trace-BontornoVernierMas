...
export function VigenciaMasivaModal({ onSubmit, onCancel, isLoading }: Props) {
  ...
  const {
    ...
    defaultValues: { materia_id: '', carrera_id: '', cohorte_id: '', desde: '', hasta: null },
  })

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-lg p-6">
        <h3 className="text-lg font-medium text-gray-900 mb-4">Modificar Vigencia (Masivo)</h3>
        <p className="text-sm text-gray-500 mb-4">Actualizá las fechas de vigencia de TODAS las asignaciones de un equipo (materia + carrera + cohorte).</p>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label htmlFor="desde" className="block text-sm font-medium text-gray-700 mb-1">Nueva vigencia desde</label>
            ...
            <div>
              <label htmlFor="hasta" className="block text-sm font-medium text-gray-700 mb-1">Nueva vigencia hasta</label>
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <button type="submit" disabled={isLoading} className="px-4 py-2 text-sm text-white bg-blue-600 rounded hover:bg-blue-700 disabled:opacity-50">
              {isLoading ? 'Actualizando…' : 'Actualizar Vigencia'}
            </button>
          </div>
        </form>
      </div>
