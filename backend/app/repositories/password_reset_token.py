...
class PasswordResetTokenRepository:
    ...
    async def get_valid_by_hash(
        self, token_hash: str, tenant_id: uuid.UUID
    ) -> PasswordResetToken | None:
        """Return a valid (non-expired, non-used) token matching the hash within the tenant.

        Joins with usuarios to enforce tenant isolation (PasswordResetToken has no
        tenant_id column; tenant scope comes from the owning user).
        Returns None if not found, already used, expired, or belonging to a different tenant.
        """
        now = datetime.now(tz=timezone.utc)
        stmt = (
            select(PasswordResetToken)
            .join(Usuario, Usuario.id == PasswordResetToken.usuario_id)
            .where(
