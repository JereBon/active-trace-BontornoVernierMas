...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    # crypto.py reads ENCRYPTION_KEY directly from os.environ (not pydantic settings).
    # When running with a .env file, pydantic-settings loads the value but doesn't
    # inject it into os.environ. We bridge that gap here so validate_key() finds it.
    import os as _os
    _os.environ.setdefault("ENCRYPTION_KEY", _settings.ENCRYPTION_KEY)
...
def create_application() -> FastAPI:
    ...
    # ── Middleware ────────────────────────────────────────────────────────────
    # CORS — origins are set via CORS_ALLOWED_ORIGINS env var.
    # allow_origins=["*"] is invalid with allow_credentials=True per the CORS
    # spec and will be rejected by browsers. Use explicit origins instead.
    from app.core.config import get_settings as _get_settings
    _cors_origins = _get_settings().CORS_ALLOWED_ORIGINS
