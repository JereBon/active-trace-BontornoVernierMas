...
class AnalisisService:
    ...
    async def get_atrasados(self, materia_id: uuid.UUID) -> list[dict[str, Any]]:
        ...
        atrasados = calcular_atrasados(entradas_dicts, cals_by_entrada, all_actividades)

        # Enrich each row with the student's decrypted email.
        # email_cifrado lives on EntradaPadron (AES-256-GCM); decrypt here so
        # downstream code (e.g. the comms page) never touches raw ciphertext.
        from app.core.crypto import decrypt  # noqa: PLC0415
        email_map = {e.id: e.email_cifrado for e in entradas}
        for row in atrasados:
            cifrado = email_map.get(row["entrada_padron_id"])
            try:
                row["email"] = decrypt(cifrado) if cifrado else None
            except Exception:
                row["email"] = None
