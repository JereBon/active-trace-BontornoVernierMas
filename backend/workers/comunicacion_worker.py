...
async def _dispatch_email(destinatario_cifrado: str, asunto: str, cuerpo: str) -> bool:
    ...
    if backend == "stub":
        ...
        return True
    logger.warning("Unknown EMAIL_BACKEND=%r — treating as failure", backend)
    return False


# Messages stuck in Enviando for longer than this are eligible for rescue.
_ENVIANDO_STALE_MINUTES = 10
...
class ComunicacionWorker:
    ...
    async def run_once(self) -> int:
        ...
        # ── Phase 0: Rescue messages stuck in Enviando ────────────────────────
        # If the worker crashed after Phase 1 committed but before Phase 2 finished,
        # those messages sit in Enviando forever because Phase 1 only picks Pendiente.
        # Any Enviando message older than _ENVIANDO_STALE_MINUTES is reset to Pendiente.
        stale_cutoff = datetime.now(tz=timezone.utc) - timedelta(minutes=_ENVIANDO_STALE_MINUTES)
        async with self._session_factory() as rescue_session:
            async with rescue_session.begin():
                rescued = await rescue_session.execute(
                    update(Comunicacion)
                    .where(
                        Comunicacion.estado == "Enviando",
                        Comunicacion.updated_at < stale_cutoff,
                        Comunicacion.deleted_at.is_(None),
                    )
                    .values(estado="Pendiente", updated_at=datetime.now(tz=timezone.utc))
                )
                if rescued.rowcount:
                    logger.warning(
                        "ComunicacionWorker rescued %d stuck Enviando message(s) → Pendiente",
                        rescued.rowcount,
                    )
